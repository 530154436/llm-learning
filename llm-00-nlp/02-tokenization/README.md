...
| 编码   | 将单词分割成字符，并应用在训练过程中学到的合并             | 从开头开始查找属于词汇表中最长的子词前缀，然后对其余部分重复此过程            | 使用训练中学到的分数找到最可能的token分割方式    |
| 代表模型 | GPT,GPT-2,RoBERTa,BART,和DeBERTa<br>开源大规模语言模型如Llama系列、Mistral、Qwen | BERT                                         | AlBERT，T5，mBART   |
