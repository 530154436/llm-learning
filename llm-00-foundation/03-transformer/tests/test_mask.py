...
# @function:
import torch
from modules.mask import create_padding_mask, create_look_ahead_mask, create_decoder_mask
...
def test_create_padding_mask():
    ...
    print(f"填充掩码({res.shape})：\n{res}")  # torch.Size([2, 1, 1, 5])
    print()
...
def test_create_look_ahead_mask():
    seq = torch.tensor([[5, 7, 9, 0, 0],
                        [8, 6, 0, 0, 0]])
    res = create_look_ahead_mask(seq.size(1))
...
def test_create_decoder_mask():
    ...
    combined_mask2 = create_decoder_mask(seq)
